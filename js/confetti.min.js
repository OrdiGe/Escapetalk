...
! function() {
    ...
    function resetParticle(particle, width, height) {
        return particle.color = colors[Math.random() * colors.length | 0], particle.x = Math.random() * width, particle.y = Math.random() * height - height, particle.diameter = 10 * Math.random() + 5, particle.tilt = 10 * Math.random() - 10, particle.tiltAngleIncrement = .07 * Math.random() + .05, particle.tiltAngle = 0, particle
    }
    ...
    function startConfettiInner() {
        ...
        null === canvas && ((canvas = document.createElement("canvas")).setAttribute("id", "confetti-canvas"), canvas.setAttribute("style", "display:block;z-index:999999;pointer-events:none;position: absolute;"), document.body.appendChild(canvas), canvas.width = width, canvas.height = height, window.addEventListener("resize", (function() {
    ...
    function drawParticles(context) {
        for (var particle, x, i = 0; i < particles.length; i++) particle = particles[i], 
        context.beginPath(), 
        context.lineWidth = particle.diameter, 
        context.strokeStyle = particle.color, 
        x = particle.x + particle.tilt, 
        context.moveTo(x + particle.diameter / 2, particle.y), 
        context.lineTo(x, (particle.y + particle.tilt + particle.diameter / 2) / 3), context.stroke()
    }
